...
方式 B: 环境变量 (适合 cron / Docker)
    set CITYBOX_ACCOUNTS=[{"token":"xxx","sign":"yyy"}]
    python sign.py
...
def load_accounts(
    ...
    # 3. 环境变量 CITYBOX_ACCOUNTS (JSON 数组)
    env_raw = os.environ.get("CITYBOX_ACCOUNTS")
    if env_raw and not accounts:
        try:
    ...
    if not accounts:
        raise ValueError(
            "未找到任何账号配置。请通过 --token / --config / 环境变量 CITYBOX_ACCOUNTS 提供账号。"
        )
    return accounts
...
# 通知 (可选, 默认只打印)
# ---------------------------------------------------------------------------
def notify(summary: str, webhook: str | None) -> None:
    """简易 webhook 通知; 失败不影响主流程。"""
    if not webhook:
        return
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--webhook", help="可选通知 webhook (POST JSON {text:...})")
    parser.add_argument(
        "--no-verify-ssl",
        action="store_true",
